                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                        {subjectEntries.map((entry, i) => (
                          <ArchiveCard key={entry.id} entry={entry} index={i}
                            ...
                            onRestore={() => setConfirmModal({ type: 'status', id: entry.id, action: 'pending' })}
...
/* ── ArchiveCard ─────────────────────────────────────────────────────────── */
function ArchiveCard({ entry, index, isExpanded, onToggle, onDelete, onAccept, onReject, onRestore, onDataset,
      ...
      <div
        ...
        <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-1">
            ...
            {entry.status === 'rejected' && (
              <ActionBtn onClick={onRestore} disabled={updating} title={t('archive.restorePending')}
                cls="border-amber-200 bg-amber-50 text-amber-600 hover:bg-amber-100 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-400 dark:hover:bg-amber-500/20">
                <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M1 4h8a4 4 0 0 1 0 8H5"/><polyline points="4 1 1 4 4 7"/></svg>
              </ActionBtn>
            )}
            {entry.status === 'accepted' && (
              <>
                <ActionBtn onClick={onRestore} disabled={updating} title={t('archive.restorePending')}
                  cls="border-gray-200 bg-gray-100 text-gray-500 hover:bg-gray-200 dark:border-white/10 dark:bg-white/5 dark:text-gray-400 dark:hover:bg-white/10">
                  <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M1 4h8a4 4 0 0 1 0 8H5"/><polyline points="4 1 1 4 4 7"/></svg>
                </ActionBtn>
...
/* ── BulkBar ─────────────────────────────────────────────────────────────── */
function BulkBar({ count, total, updating, onSelectAll, onClear, onMoveAccepted, onMoveRejected, onRestore }) {
  const { t } = useTranslation();
  return (
    <div className="flex items-center gap-3 flex-wrap px-4 py-2.5 mb-5 rounded-xl"
      ...
      <div className="flex items-center gap-1.5">
        ...
        <BulkBtn onClick={onRestore} disabled={updating} title={t('archive.restorePending')}
          cls="border-gray-200 bg-white text-gray-600 hover:bg-gray-100 dark:border-white/10 dark:bg-white/5 dark:text-gray-400 dark:hover:bg-white/10">
          <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M1 4h8a4 4 0 0 1 0 8H5"/><polyline points="4 1 1 4 4 7"/></svg>
          {t('archive.restore')}
        </BulkBtn>
      </div>
