...
export default function ExamBuilderPage() {
  ...
  const DRAFT_KEY = 'exam_builder_draft';
  ...
  useEffect(() => {
    try {
      ...
      if (!savedHeader || !Object.keys(savedHeader).length) {
        try {
          const { contexts = [] } = JSON.parse(localStorage.getItem('exam_teaching_profile') || '{}');
          const autoHeader = {};
          // Fill from first teaching context if available
          ...
          // Fall back to user profile fields if not already set
  ...
  /* ── Frames ── */
  const addFrameToItem = (examId) =>
    setExamItems(prev => prev.map(i => i._examId !== examId ? i :
      { ...i, frames: [...(i.frames || []), { id: Date.now() + Math.random(), title: '', lines: 6 }] }));

  const removeFrameFromItem = (examId, frameId) =>
    setExamItems(prev => prev.map(i => i._examId !== examId ? i :
      { ...i, frames: (i.frames || []).filter(f => f.id !== frameId) }));

  const updateFrameField = (examId, frameId, key, val) =>
    setExamItems(prev => prev.map(i => i._examId !== examId ? i :
      { ...i, frames: (i.frames || []).map(f => f.id === frameId ? { ...f, [key]: val } : f) }));
  ...
  const exportPDF = () => {
    ...
    style.id = '__exam_print_style';
    style.innerHTML = `@media print { body > *:not(#exam-print-root) { display: none !important; } #exam-print-root { display: block !important; } .exam-page { page-break-after: always; margin: 0 !important; box-shadow: none !important; } @page { size: A4; margin: 15mm 20mm; } }`;
    document.head.appendChild(style);
    const clone = pagesContainerRef.current.cloneNode(true);
    clone.id = 'exam-print-root';
    clone.style.cssText = 'display:none;';
    document.body.appendChild(clone);
    setTimeout(() => { window.print(); document.head.removeChild(style); document.body.removeChild(clone); }, 100);
  };
...
<head><meta charset='utf-8'><style>
  ...
  .exercise{margin-bottom:20pt;page-break-inside:avoid}
...
${examItems.map((item, i) => buildExerciseHtml(item, i)).join('')}
  ...
  const profileHints = useMemo(() => {
    try {
      ...
      // Also fold in user profile fields
  ...
  }, [user]);  // re-compute only on mount — localStorage doesn't trigger re-renders
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-[#00053B]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Toolbar */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 dark:border-white/[0.07] flex-shrink-0 bg-white dark:bg-[#00053B]">
          <div className="flex items-center gap-3">
            {/* Bank toggle button */}
            <button onClick={() => setShowBank(b => !b)} title="Exercise Bank"
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[12px] font-semibold transition-all duration-200
                ${showBank
                  ? 'bg-sky-500 border-sky-500 text-white shadow-lg shadow-sky-500/25'
                  : 'bg-transparent border-sky-200 dark:border-sky-500/30 text-sky-600 dark:text-sky-300 hover:bg-sky-50/60 dark:hover:bg-sky-500/10'}`}>
              <Icon src={IMG.bank} size={15} />
              <span>{t('examBuilder.bank')}</span>
              {bankItems.length > 0 && (
                <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-md ${showBank ? 'bg-white/20 text-white' : 'bg-sky-100/60 dark:bg-sky-500/15 text-sky-600 dark:text-sky-300'}`}>
                  {filteredBank.length}
                </span>
              )}
            </button>
            ...
            <div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-500/15 border border-blue-200 dark:border-blue-500/30 flex items-center justify-center">
              <Icon src={IMG.exam} size={18} />
            </div>
            <div>
              <p className="text-[13px] font-bold text-gray-900 dark:text-white">{t('examBuilder.title')}</p>
              <p className="text-[10px] text-gray-400 dark:text-white/40 font-mono">
                {examItems.length > 0
                  ...
                  : ''}
                <span className="text-blue-500 dark:text-blue-400 font-semibold">{pageCount} page{pageCount !== 1 ? 's' : ''}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            {examItems.length > 0 && (
              <button onClick={clearDraft} title="مسح المسودة"
                className="w-8 h-8 rounded-lg flex items-center justify-center border border-gray-200 dark:border-white/10 text-gray-400 dark:text-white/30 hover:text-red-500 hover:border-red-200 dark:hover:border-red-500/30 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all">
            ...
            <div className="h-5 w-px bg-gray-200 dark:bg-white/10" />
            <ToolbarBtn onClick={() => setShowHeaderModal(true)} icon={IMG.edit} label={t('examBuilder.header')}
              cls="border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-100 dark:border-white/10 dark:bg-white/5 dark:text-white/50 dark:hover:bg-white/10" />
            <ToolbarBtn onClick={finalizeExam} icon={IMG.accept} label={t('examBuilder.finalize')}
              cls="border-sky-200 bg-transparent text-sky-600 hover:bg-sky-50/60 dark:border-sky-500/30 dark:bg-transparent dark:text-sky-300 dark:hover:bg-sky-500/10" />
            <ToolbarBtn onClick={exportPDF} label="PDF" svgIcon={
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
            } cls="border-red-200 bg-red-50 text-red-600 hover:bg-red-100 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20" />
            <ToolbarBtn onClick={exportWord} label="Word" svgIcon={
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
            } cls="border-blue-200 bg-blue-50 text-blue-600 hover:bg-blue-100 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-400 dark:hover:bg-blue-500/20" />
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* ── Exercise Bank — pushes document ──────────────────────── */}
          <div className={`flex-shrink-0 overflow-hidden print:hidden transition-[width] duration-300 ease-out ${showBank ? 'w-[300px]' : 'w-0'}`}>
            <div className="w-[300px] h-full flex flex-col bg-white dark:bg-[#0d1426] border-r border-gray-200 dark:border-white/[0.07]">

              {/* Panel header */}
              <div className="px-5 pt-5 pb-4 border-b border-gray-100 dark:border-white/[0.07] flex-shrink-0">
                ...
                {/* Search */}
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 mb-3">
                  ...
                  <div className="text-center py-12 px-4">
                    ...
                    {bankItems.length === 0 && (
                      <Link to="/archive" onClick={() => setShowBank(false)}
                        className="mt-3 inline-flex items-center gap-1 text-[11px] text-blue-500 dark:text-blue-400 hover:underline">
                        Open Archive →
                      </Link>
                    )}
                  </div>
              ...
              <div className="px-4 py-3 border-t border-gray-100 dark:border-white/[0.07] flex-shrink-0">
                <p className="text-[10px] text-gray-400 dark:text-white/30 font-mono text-center">
                  Drag onto the document · or click <span className="text-emerald-500">+</span>
                </p>
              </div>
            </div>
          </div>
          ...
          {/* ── Canvas ────────────────────────────────────────────────── */}
          <div className="flex-1 overflow-y-auto bg-[#525659] dark:bg-[#111827] py-8 px-4 relative">
            {examItems.length > 0 && (
                      ...
                      return (
                        <div key={examId} ref={el => { if (el) exerciseElRefs.current[examId] = el; }}>
                          <ExamExerciseItem item={item} index={idx}
                            ...
                            onAddFrame={() => addFrameToItem(item._examId)}
                            onRemoveFrame={frameId => removeFrameFromItem(item._examId, frameId)}
                            onUpdateFrame={(frameId, key, val) => updateFrameField(item._examId, frameId, key, val)}
...
/* ── ToolbarBtn (icon-only, label as tooltip) ───────────────────────────── */
function ToolbarBtn({ onClick, icon, svgIcon, label, cls }) {
  return (
    <button onClick={onClick} title={label}
      className={`w-8 h-8 flex items-center justify-center rounded-lg border transition-all ${cls}`}>
      {icon    && <Icon src={icon} size={15} />}
      {svgIcon && svgIcon}
    </button>
  );
}
...
/* ── BankItem ───────────────────────────────────────────────────────────── */
function BankItem({ item, inExam, onAdd, onDragStart }) {
  return (
    <div draggable onDragStart={e => onDragStart(e, item._bankId)}
      className={`group flex items-start gap-2.5 p-3 rounded-2xl border cursor-grab active:cursor-grabbing transition-all
        ${inExam
          ? 'border-sky-200/50 dark:border-sky-500/20 bg-sky-50/30 dark:bg-sky-500/5 opacity-50'
          : 'border-gray-200 dark:border-white/[0.07] bg-gray-50 dark:bg-white/[0.03] hover:border-blue-300 dark:hover:border-blue-500/30 hover:bg-blue-50 dark:hover:bg-blue-500/5'}`}>
      ...
      <button onClick={onAdd} disabled={inExam} title={inExam ? 'Already added' : 'Add to exam'}
        className={`flex-shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-xs font-bold transition-all
          ${inExam
            ...
            : 'border border-gray-300 dark:border-white/10 text-gray-400 dark:text-white/40 hover:border-blue-400 hover:text-blue-600 dark:hover:border-blue-500/50 dark:hover:text-blue-300'}`}>
        {inExam
...
function ExamExerciseItem({ item, index, isDragOver, onDragStart, onDragOver, onDrop, onDragEnd,
  ...
  onAddFrame, onRemoveFrame, onUpdateFrame, isFocused }) {
  ...
  return (
    <div draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop} onDragEnd={onDragEnd}
      ...
      <div className="absolute -right-24 top-0 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity font-sans print:hidden">
        ...
        <button onClick={() => triggerInsert(null)} title="Ajouter une image"
        ...
        {/* Add frame */}
        <button onClick={onAddFrame} title="Add diagram frame"
          ...
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>
        </button>
      ...
      {(item.frames || []).map(frame => (
        <div key={frame.id} style={{ marginTop: '10pt', pageBreakInside: 'avoid' }} className="relative group/frame">
          {/* Clean empty box — no lines inside */}
          <div style={{ border: '1.5px solid #374151', height: `${(Number(frame.lines) || 6) * 14}pt` }} />
          {/* Editable title below the box */}
          ...
          {/* Height adjuster (screen only) */}
          <div className="absolute top-1 right-1 flex items-center gap-1 opacity-0 group-hover/frame:opacity-100 transition-opacity print:hidden font-sans">
            <button onClick={() => onUpdateFrame(frame.id, 'lines', Math.max(3, (Number(frame.lines) || 6) - 1))}
            ...
            <span className="text-[9px] text-gray-400 font-mono">{frame.lines}</span>
            <button onClick={() => onUpdateFrame(frame.id, 'lines', Math.min(30, (Number(frame.lines) || 6) + 1))}
              className="w-4 h-4 text-[10px] rounded border border-gray-300 bg-white text-gray-500 flex items-center justify-center hover:bg-gray-100">+</button>
            <button onClick={() => onRemoveFrame(frame.id)}
              className="w-4 h-4 rounded-full bg-red-500 text-white flex items-center justify-center ml-1">
...
function buildExerciseHtml(item, index) {
  const qHtml = (item.questions || []).map((q, qi) => {
    const data    = q.data || q;
    const stem    = data.stem || data.question || data.title || '';
    ...
    return `<div style="margin:6pt 0 6pt 16pt"><p style="font-weight:600;font-size:11pt">Q${qi+1}. ${stem}</p>${choices}${parts}</div>`;
  }).join('');
  return `<div style="margin-bottom:20pt;page-break-inside:avoid">
<div style="font-weight:bold;font-size:13pt;border-bottom:1px solid #333;padding-bottom:4pt;margin-bottom:8pt">
Exercice ${index+1}${item.title ? ' — ' + item.title : ''}
