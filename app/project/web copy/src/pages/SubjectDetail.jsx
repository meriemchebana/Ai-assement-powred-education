          ...
          <div className={`glass-card p-5 mb-6 relative overflow-hidden border ${tabMeta?.tw.border}`}>
            ...
            <form onSubmit={handleCreate} className="flex flex-col gap-2.5">
              ...
              <div className="flex gap-2 justify-end">
                <button type="button" onClick={() => setShowForm(false)} className="btn-secondary text-xs px-4 py-2">{t('subjectDetail.cancel')}</button>
            ...
            <div className="flex flex-col gap-3">
              {items.map((item, index) => (
                <TimelineCard
                  ...
                  onGenerateAI={() => {
                    if (activeTab === 'exams') setGenerateModal({ open: true, examId: item.id });
          ...
          <div className="flex-1 min-w-0">
                      ...
                      <div key={pdf.id} className="flex items-center justify-between px-3 py-2 bg-black/5 dark:bg-white/[0.06] rounded-xl border border-light-border dark:border-white/[0.07]">
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          ...
                          <button onClick={() => onDeletePDF(pdf.id, pdf.filename)}
                            className="text-[11px] text-light-muted dark:text-white/30 px-2 py-1 rounded-lg border border-transparent font-mono hover:text-red-500 hover:border-red-400/30 hover:bg-red-500/[0.06] transition-all">
                            {t('subjectDetail.del')}
                          </button>
                        </div>
                      </div>
            ...
            {uploadingFor === item.id && (
              <div className="flex gap-2 items-center mt-2.5 pl-[23px]">
                <input type="file" accept=".pdf" onChange={e => onFileChange(e.target.files[0])}
                  className="text-xs text-light-muted dark:text-white/50 font-mono flex-1 max-w-[200px]" />
          ...
          <div className="flex gap-1.5 flex-shrink-0 items-start">
            ...
            {activeTab === 'exams' && (
              <button onClick={onGenerateAI}
                className="px-2.5 py-1.5 rounded-lg text-[11px] font-mono font-semibold transition-all bg-light-primary/10 border border-light-primary/25 text-light-primary hover:bg-light-primary/20 hover:border-light-primary/50 dark:bg-violet-500/10 dark:border-violet-500/25 dark:text-violet-400 dark:hover:bg-violet-500/20 dark:hover:border-violet-500/50">
                <img src="/image/3775742-bulb-creative-idea-light-thunder_108969.png" alt="" style={{ width: 20, height: 20, objectFit: 'contain', verticalAlign: 'middle' }} /> AI
              </button>
            )}
