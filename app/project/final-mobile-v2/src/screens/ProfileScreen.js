...
/* ── LocalStorage helper for contexts ────────────────────────────────────── */
const loadContexts = async () => {
  try {
    const raw = await AsyncStorage.getItem(TEACHING_PROFILE_KEY);
    return raw ? JSON.parse(raw).contexts || [] : [];
  } catch { return []; }
};

const saveContexts = async (contexts) => {
  await AsyncStorage.setItem(TEACHING_PROFILE_KEY, JSON.stringify({ contexts }));
};
