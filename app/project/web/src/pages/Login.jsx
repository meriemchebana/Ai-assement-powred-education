...
export default function Login() {
  ...
  const gsiInitialized  = useRef(false);  // initialize() must be called only once per page load
  ...
  // initialize() is called only once; renderButton() is called whenever the button
  // container changes (theme toggle or login↔register tab switch).
  useEffect(() => {
    if (!GOOGLE_CLIENT_ID || !googleBtnRef.current) return;

    const renderBtn = () => {
      if (!googleBtnRef.current) return;
      // Initialize only once per page load to avoid GSI_LOGGER warnings
    ...
    // Script already loaded (cached or previous mount) — render directly
    if (window.google) {
      renderBtn();
      return;
    }

    // Re-use existing script tag if another mount already added it
