...
export default function Subjects() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const payload = { ...form };
      if (formImage) payload.cover_image = formImage;
    ...
    <div className="flex h-screen overflow-hidden bg-light-bg dark:bg-dark-bg">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative z-10">
        ...
        <div className="flex-1 overflow-y-auto p-8 relative z-10">
          ...
          {/* create modal is rendered at root level below */}
      ...
      {showForm && (
        <NewSubjectModal
          ...
          onPickImage={() => setShowFormPicker(true)}
          onPickIcon={setFormIcon}
      ...
      {showFormPicker && (
        <ImagePickerModal
          currentImage={formImage || subjectImg(form.name)}
          onSelect={url => { setFormImage(url); setShowFormPicker(false); }}
...
/* ════════════════════════════════════════════════════════
   NEW SUBJECT MODAL
   ════════════════════════════════════════════════════════ */
function NewSubjectModal({ form, formImage, formIcon, onFormChange, onPickImage, onPickIcon, onSubmit, onClose, t }) {
  const previewImg = formImage || subjectImg(form.name);
  ...
  return (
    ...
    >
      ...
      >
        {/* Cover image */}
        <div className="relative h-44 group/cover cursor-pointer" onClick={onPickImage}>
          <img src={previewImg} alt="cover" className="w-full h-full object-cover" />
          ...
          <div className="absolute inset-0 bg-black/0 group-hover/cover:bg-black/30 transition-colors flex items-center justify-center">
            <span className="opacity-0 group-hover/cover:opacity-100 transition-opacity flex items-center gap-2 bg-black/50 backdrop-blur-md px-4 py-2 rounded-xl text-white text-xs font-bold border border-white/20">
              <img src="/src/assets/icon-folder.png" alt="" style={{ width: 14, height: 14, objectFit: 'contain', filter: 'brightness(0) invert(1)' }} />
        ...
        <div className="p-6">
          <div className="flex items-center gap-3 mb-5">
            <div className="w-8 h-8 rounded-lg bg-light-primary/10 dark:bg-dark-primary/20 flex items-center justify-center">
              <img src="/image/3775742-bulb-creative-idea-light-thunder_108969.png" alt="" style={{ width: 18, height: 18, objectFit: 'contain' }} />
            </div>
            <h3 className="text-base font-extrabold text-light-text dark:text-white">{t('subjects.newSubject')}</h3>
            <button onClick={onClose} className="ml-auto text-light-muted dark:text-white/40 hover:text-red-400 transition-colors text-xl leading-none">✕</button>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-3">
            ...
            {/* Icon picker */}
            <div>
              <p className="text-[10px] font-mono uppercase tracking-widest text-light-muted dark:text-white/40 mb-2">Icon</p>
                ...
                {ICONS.map(icon => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all border-2
                      ${formIcon === icon
                        ? 'border-light-primary dark:border-teal-400 bg-light-primary/10 dark:bg-teal-400 scale-110 shadow-md'
                        : 'border-transparent bg-black/5 dark:bg-teal-400/30 hover:bg-black/10 dark:hover:bg-teal-400/50'
                      }`}>
                    <img src={`/image/${icon}`} alt="" style={{ width: 24, height: 24, objectFit: 'contain' }} />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button type="submit" className="btn-primary flex-1 text-sm py-2.5 flex items-center justify-center gap-2">
