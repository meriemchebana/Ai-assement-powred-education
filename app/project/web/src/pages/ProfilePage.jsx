...
const loadContexts = (userId) => {
  if (!userId) return [];
  try { return JSON.parse(localStorage.getItem(`exam_teaching_profile_${userId}`) || '{}').contexts || []; }
  catch { return []; }
};

const saveContexts = (ctxs, userId) => {
  if (!userId) return;
  localStorage.setItem(`exam_teaching_profile_${userId}`, JSON.stringify({ contexts: ctxs }));
};
...
export default function ProfilePage() {
  ...
  /* ── Teaching contexts state (empty until user.id is known) ── */
  ...
  // Load contexts once user.id is available (user-scoped key)
  useEffect(() => {
    if (!user?.id) return;
    setContexts(loadContexts(user.id));
  }, [user?.id]);

  // Auto-save contexts to localStorage whenever they change (user-scoped)
  useEffect(() => {
    if (!user?.id) return;
          ...
          <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-light-primary to-blue-500 dark:from-blue-700 dark:to-dark-bg shadow-2xl mb-10 p-8 md:p-10">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center md:items-end gap-6">
              ...
              <div className="text-white text-center md:text-left flex-1">
                <p className="text-[11px] font-mono uppercase tracking-[2px] text-white/60 mb-1">{t('profile.teacherProfile')}</p>
                ...
                <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                  ...
                  {form.institution && (
                    <span className="text-xs bg-white/20 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 font-medium">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:'inline',marginRight:4,verticalAlign:'middle'}}><line x1="3" y1="22" x2="21" y2="22"/><line x1="6" y1="18" x2="6" y2="11"/><line x1="10" y1="18" x2="10" y2="11"/><line x1="14" y1="18" x2="14" y2="11"/><line x1="18" y1="18" x2="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg>{form.institution}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-4 md:flex-col md:gap-3 md:text-right">
                <div className="text-center md:text-right">
                  ...
                  <p className="text-[11px] text-white/60 font-medium uppercase tracking-wide">{t('profile.statsSubjects')}</p>
                </div>
                <div className="text-center md:text-right">
                  <p className="text-3xl font-extrabold text-white">{stats.exams}</p>
                  <p className="text-[11px] text-white/60 font-medium uppercase tracking-wide">{t('profile.statsExams')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="glass-card p-6 relative overflow-hidden md:col-span-2">
              ...
              <h3 className="text-base font-extrabold text-light-text dark:text-white mb-5 flex items-center gap-2">
                <span className="w-7 h-7 rounded-lg bg-light-primary/10 dark:bg-dark-primary/20 flex items-center justify-center text-light-primary dark:text-sky-300">
            ...
            <div className="glass-card p-6 relative overflow-hidden">
              ...
              <h3 className="text-base font-extrabold text-light-text dark:text-white mb-5 flex items-center gap-2">
                <span className="w-7 h-7 rounded-lg bg-light-primary/10 dark:bg-blue-500/10 flex items-center justify-center text-light-primary dark:text-blue-400">
              ...
              <div className="space-y-4">
                ...
                <FormField label={t('profile.position')}>
                  <select value={form.position} onChange={e => setForm(f => ({...f, position: e.target.value}))}
                    ...
                    <option value="">{t('profile.selectPosition')}</option>
            ...
            <div className="glass-card p-6 relative overflow-hidden md:col-span-2">
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-extrabold text-light-text dark:text-white flex items-center gap-2">
                  <span className="w-7 h-7 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-400">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                  </span>
                  {t('profile.teachingContexts')}
                    ...
                    {editingCtxId !== ctx.id && (
                      <div className="group flex items-start gap-3 px-4 py-3 rounded-xl border border-gray-200 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.03] hover:border-blue-200 dark:hover:border-blue-500/25 hover:bg-blue-50/40 dark:hover:bg-blue-500/5 transition-all">
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-1.5">
                            <span className="text-[13px] font-bold text-gray-800 dark:text-white truncate">
                              {ctx.institution || <span className="italic text-gray-400 dark:text-white/30">{t('profile.noInstitution')}</span>}
                            </span>
                            {ctx.faculty && (
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 mt-0.5">
                          <button type="button" onClick={() => openEditCtx(ctx)} title={t('common.edit')}
                          ...
                          <button type="button" onClick={() => deleteCtx(ctx.id)} title={t('common.delete')}
  ...
  return (
    <div className="rounded-xl border border-blue-200 dark:border-blue-500/30 bg-blue-50/50 dark:bg-blue-500/5 p-4 space-y-4">
      <p className="text-[11px] font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider mb-1">
        {isNew ? t('profile.newContext') : t('profile.editContext')}
      </p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-1.5">{t('profile.institution')}</label>
        ...
        <div>
          <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-1.5">{t('profile.facultyDept')}</label>
          <input value={draft.faculty} onChange={e => onFieldChange('faculty', e.target.value)}
            placeholder={t('profile.facultyPlaceholder')}
            className="w-full px-3 py-2 rounded-lg bg-white dark:bg-white/[0.06] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-[12px] placeholder-gray-300 dark:placeholder-white/20 outline-none focus:border-blue-400 dark:focus:border-blue-500/50 transition-all" />
        </div>
        <div>
          <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-1.5">{t('profile.speciality')}</label>
        ...
        <div>
          <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-1.5">{t('profile.academicYear')}</label>
          <input value={draft.academic_year} onChange={e => onFieldChange('academic_year', e.target.value)}
            placeholder="2024/2025"
            className="w-full px-3 py-2 rounded-lg bg-white dark:bg-white/[0.06] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-[12px] placeholder-gray-300 dark:placeholder-white/20 outline-none focus:border-blue-400 dark:focus:border-blue-500/50 transition-all" />
        </div>
        <div>
          <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-1.5">{t('profile.defaultSession')}</label>
      ...
      {/* Levels */}
      <div>
        <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-2">{t('profile.levels')}</label>
        <div className="flex flex-wrap gap-1.5">
      ...
      {/* Subjects tag input */}
      <div>
        <label className="block text-[10px] font-bold text-gray-500 dark:text-white/65 uppercase tracking-wider mb-2">{t('profile.subjectTags')}</label>
        <div className="flex flex-wrap gap-1.5 p-2.5 rounded-xl bg-white dark:bg-white/[0.04] border border-gray-200 dark:border-white/10 focus-within:border-blue-400 dark:focus-within:border-blue-500/50 transition-all min-h-[44px]">
          ...
          <input value={tagInput} onChange={e => setTagInput(e.target.value)}
            ...
            placeholder={draft.subjects.length === 0 ? t('profile.addSubjectHint') : ''}
